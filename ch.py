    ...
    with Client(conn, request_timeout=5, config=config) as client:
        ...
        try:
            ...
            if response.positive:
                ecu_info = response.service_data.values[0xF187]
                logging.info(f"ECU information (DID {hex(0xF187)}): {ecu_info}")
                #display_text(f"DID {hex()}: {ecu_info}")
                #time.sleep(0.5)
            else:
                display_text("No Data Received")
        except Exception as e:
            logging.error(f"Error reading ECU information (DID {hex(0xF187)}): {e}")
            #display_text("ECU Read Error")

        try:
            response = client.read_data_by_identifier(0xF193)
            if response.positive:
                ecu_info = response.service_data.values[0xF193]
                logging.info(f"ECU information (DID {hex(0xF193)}): {ecu_info}")
                #display_text(f"DID {hex()}: {ecu_info}")
                #time.sleep(0.5)
            else:
                display_text("No Data Received")
        except Exception as e:
            logging.error(f"Error reading ECU information (DID {hex(0xF193)}): {e}")
            #display_text("ECU Read Error")

        try:
            response = client.read_data_by_identifier(0xF102)
            if response.positive:
                ecu_info = response.service_data.values[0xF102]
                logging.info(f"ECU information (DID {hex(0xF102)}): {ecu_info}")
                #display_text(f"DID {hex()}: {ecu_info}")
                #time.sleep(0.5)
            else:
                display_text("No Data Received")
        except Exception as e:
            logging.error(f"Error reading ECU information (DID {hex(0xF102)}): {e}")
            #display_text("ECU Read Error")

        try:
            response = client.read_data_by_identifier(0xF120)
            if response.positive:
                ecu_info = response.service_data.values[0xF120]
                logging.info(f"ECU information (DID {hex(0xF120)}): {ecu_info}")
                #display_text(f"DID {hex()}: {ecu_info}")
                #time.sleep(0.5)
            else:
                display_text("No Data Received")
        except Exception as e:
            logging.error(f"Error reading ECU information (DID {hex(0xF120)}): {e}")
            #display_text("ECU Read Error")

        try:
            response = client.read_data_by_identifier(0xF18b)
            if response.positive:
                ecu_info = response.service_data.values[0xF18b]
                logging.info(f"ECU information (DID {hex(0xF18b)}): {ecu_info}")
                #display_text(f"DID {hex()}: {ecu_info}")
                #time.sleep(0.5)
            else:
                display_text("No Data Received")
        except Exception as e:
            logging.error(f"Error reading ECU information (DID {hex(0xF18b)}): {e}")
            #display_text("ECU Read Error")

        try:
            response = client.read_data_by_identifier(0xF1b1)
            if response.positive:
                ecu_info = response.service_data.values[0xF1b1]
                logging.info(f"ECU information (DID {hex(0xF1b1)}): {ecu_info}")
                #display_text(f"DID {hex()}: {ecu_info}")
                #time.sleep(0.5)
            else:
                display_text("No Data Received")
        except Exception as e:
            logging.error(f"Error reading ECU information (DID {hex(0xF1b1)}): {e}")
            #display_text("ECU Read Error")

        try:
            response = client.read_data_by_identifier(0xF100)
            if response.positive:
                ecu_info = response.service_data.values[0xF100]
                logging.info(f"ECU information (DID {hex(0xF100)}): {ecu_info}")
                #display_text(f"DID {hex()}: {ecu_info}")
                #time.sleep(0.5)
            else:
                display_text("No Data Received")
        except Exception as e:
            logging.error(f"Error reading ECU information (DID {hex(0xF100)}): {e}")
            #display_text("ECU Read Error")

        try:
            response = client.read_data_by_identifier(0xF101)
            if response.positive:
                ecu_info = response.service_data.values[0xF101]
                logging.info(f"ECU information (DID {hex(0xF101)}): {ecu_info}")
                #display_text(f"DID {hex()}: {ecu_info}")
                #time.sleep(0.5)
            else:
                display_text("No Data Received")
        except Exception as e:
            logging.error(f"Error reading ECU information (DID {hex(0xF101)}): {e}")
            #display_text("ECU Read Error")
