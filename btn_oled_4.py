...
menu_combinations = {
    (BTN_FIRST, BTN_FIRST): "Option A",
    (BTN_SECOND, BTN_SECOND): "Option B",
    (BTN_FIRST, BTN_SECOND): "Option C",
    (BTN_SECOND, BTN_FIRST): "Option D",
    (BTN_FIRST,): "Option E",
    (BTN_SECOND,): "Option F"
}
selected_sequence = []
...
try:
    while True:
        ...
        if len(selected_sequence) in [1, 2]:  # Allow single or double presses
            selected_option = menu_combinations.get(tuple(selected_sequence), "Unknown Option")
            display_text(f"Selected: {selected_option}")
            time.sleep(1)
        
        if GPIO.input(BTN_ENTER) == GPIO.LOW and selected_option:
        ...
        time.sleep(0.2)  # General debounce delay
