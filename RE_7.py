        ...
        while True:
            if GPIO.input(BUTTON_PIN) == GPIO.LOW:
                ...
                try:
                    ...
                    client.change_session(DiagnosticSessionControl.DefaultSession)
                    print("Default Session Activated.")
                    status = "Pass"
                except Exception as e:
                    print(f"Failed to switch to Default Session: {e}")
                    status = "Fail"
                report_data.append({"timestamp": timestamp, "action": "Default Session (0x10 0x01)", "status": status})
                time.sleep(0.5)
                ...
                try:
                    ...
                    status = "Fail"
                report_data.append({"timestamp": timestamp, "action": "Extended Session (0x10 0x03)", "status": status})
                time.sleep(0.5)
                ...
                try:
                    ...
                    print(f"Error: {e}")
                    status = "Fail"
                report_data.append({"timestamp": timestamp, "action": "Read DID (0xF190)", "status": status})
