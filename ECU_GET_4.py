...
# Function to log all received CAN frames
def log_can_frames(bus):
    logging.info("Logging all incoming CAN frames")
    while True:
        msg = bus.recv(timeout=1)
        if msg:
            logging.info(f"Received CAN ID: {hex(msg.arbitration_id)}, Data: {msg.data.hex()}")
            if msg.data[0] == 0x30:
                logging.info("Flow Control Frame (0x30) detected!")
